import { getContent } from '../data/content.js'

export const pricing = () => `
  <section class="section-space bg-neutral-100">
    <div class="container-shell">
      <div class="text-center">
        <p class="text-sm font-semibold uppercase tracking-[0.2em] text-brand-accent-600">
          ${getContent().pricing.eyebrow}
        </p>
        <h2 class="mt-3 text-3xl font-semibold text-brand-navy-900 sm:text-4xl">
          ${getContent().pricing.title}
        </h2>
      </div>
      <div class="mt-10 grid gap-8 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
        <div class="space-y-4">
          <h3 class="text-2xl font-semibold text-brand-navy-900">
            ${getContent().pricing.introTitle}
          </h3>
          <p class="text-sm text-neutral-600 sm:text-base">
            ${getContent().pricing.introText}
          </p>
          <ul class="space-y-2 text-sm text-neutral-700">
            ${getContent()
              .pricing.introHighlights.map((item) => `<li>• ${item}</li>`)
              .join('')}
          </ul>
        </div>
        <div class="pricing-gallery">
          <div class="pricing-image overflow-hidden rounded-2xl" style="--float-delay: 0s">
            <img
              src="${getContent().pricing.gallery[0].src}"
              alt="${getContent().pricing.gallery[0].alt}"
              class="h-full w-full object-contain"
              loading="lazy"
            />
          </div>
        </div>
      </div>
      <div class="mt-12 grid gap-6 lg:grid-cols-3">
        ${getContent().pricing.plans.map(
          (plan, index) => `
            <article class="card ${
              plan.highlight ? 'border-brand-accent-200 shadow-md' : ''
            }" data-reveal style="--reveal-delay: ${index * 80}ms">
              <p class="text-sm font-semibold text-brand-navy-900">${plan.name}</p>
              <div class="mt-4 space-y-2 text-sm text-neutral-600">
                <p class="text-3xl font-semibold text-brand-navy-900">${plan.price}</p>
                <p class="text-xs uppercase tracking-[0.15em]">${plan.priceNote}</p>
                <p class="text-xs text-neutral-500">${plan.monthlyAlt}</p>
              </div>
              <p class="mt-3 text-sm text-neutral-600">${plan.description}</p>
              <ul class="mt-4 space-y-2 text-sm text-neutral-700">
                ${plan.features.map((feature) => `<li>• ${feature}</li>`).join('')}
              </ul>
              <a
                class="pill-btn mt-6 w-full justify-center bg-brand-accent-500 text-white hover:bg-brand-accent-600"
                href="/company.html#contact"
              >
                ${getContent().pricing.cta}
              </a>
            </article>
          `
        ).join('')}
      </div>
      <div class="mt-8 text-center text-xs text-neutral-600">
        <p>${getContent().pricing.minimumNote}</p>
        <p>${getContent().pricing.setupNote}</p>
        <p>${getContent().pricing.osNote}</p>
      </div>
    </div>
  </section>
`
